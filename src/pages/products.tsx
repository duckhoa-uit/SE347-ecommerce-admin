...
const Products = () => {
  ...
  const handleAddEditProduct = async (product: ProductPayload) => {
    let productImages: Media[] = []
    try {
      const files = product.images.filter(image => image instanceof File) as File[]
      const uploadedFiles = (await mediaApi.uploadFiles(files)).data
      productImages = product.images.map(image => {
        if (image instanceof File) {
          const uploadedImg = uploadedFiles.shift()
      ...
      if (editProduct?._id) {
        try {
          await productApi
            .update(editProduct._id, { ...product, images: productImages })
            .then(res => {
        ...
        try {
          await productApi.add({ ...product, images: productImages }).then(res => {
            if (!productList) return

            const addedProduct = res.data
            if (addedProduct) {
              const newProductList = [addedProduct, ...productList].slice(0, pagination.pageSize)
          ...
          console.log('🚀 ~ file: products.tsx:129 ~ handleAddEditProduct ~ error', error)
          // enqueueSnackbar(error.message, {
          //    variant: 'error'
          // })
        }
